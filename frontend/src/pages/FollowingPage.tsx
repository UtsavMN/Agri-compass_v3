...
export const FollowingPage = () => {
  ...
  useEffect(() => {
    if (!userId) return;
    apiGet(`/api/follows/${userId}/following`)
  ...
  return (
    <div className="pt-20 min-h-screen bg-[#0A0A0A] max-w-2xl mx-auto px-4">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-[#F5F0E8] font-bold text-xl">Following</h1>
      </div>
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <p className="text-[#F5F0E8]/30 text-center text-xs py-8 font-semibold uppercase tracking-widest">Not following anyone yet</p>
        ) : (
          users.map(user => (
